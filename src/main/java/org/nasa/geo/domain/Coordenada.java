 ...
 * {@link CoordenadaForaDaTerraException} com o valor recusado na mensagem. Para a origem
...
public record Coordenada(double latitude, double longitude) {
    ...
    public Coordenada {
        if (latitude < LAT_MIN || latitude > LAT_MAX) {
            throw new CoordenadaForaDaTerraException("latitude", latitude, "[-90, 90]");
        }
        if (longitude < LON_MIN || longitude > LON_MAX) {
            throw new CoordenadaForaDaTerraException("longitude", longitude, "[-180, 180]");
        }
        if (Double.isNaN(latitude) || Double.isNaN(longitude)) {
            throw new CoordenadaForaDaTerraException("coordenada com NaN nao e um ponto na Terra");
        }
    }
     ...
     * presente mas fora do intervalo ⇒ {@link CoordenadaForaDaTerraException}: dado ausente e
